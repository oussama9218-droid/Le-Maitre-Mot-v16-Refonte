...
{{
  "exercises": [
    ...
    try:
        ...
        # Clean JSON to fix common AI generation issues
        # Fix common escape sequence problems
        json_content = json_content.replace('\\\\', '\\')  # Fix double backslashes
        json_content = json_content.replace('\\n', ' ')    # Replace literal \n with space
        json_content = json_content.replace('\\t', ' ')    # Replace literal \t with space
        json_content = json_content.replace('\\"', '"')    # Fix escaped quotes in strings
        ...
        try:
            ...
            try:
                data = json.loads(json_content)
                logger.info("JSON recovered after aggressive cleaning")
            except json.JSONDecodeError as e2:
                logger.error(f"JSON still invalid after cleaning: {e2}")
