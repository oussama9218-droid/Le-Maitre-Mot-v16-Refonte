...
{{
  "exercises": [
    ...
    try:
        ...
        # Clean JSON to fix common AI generation issues - TARGETED APPROACH
        # Only fix specific known problems without breaking valid content
        try:
            ...
            # TARGETED fixes for common AI issues
            # 1. Fix literal newlines and tabs (not escaped ones)
            json_content = json_content.replace('\\n', '\\\\n')  # Properly escape \n
            json_content = json_content.replace('\\t', '\\\\t')  # Properly escape \t
            json_content = json_content.replace('\\r', '\\\\r')  # Properly escape \r
            ...
            try:
                data = json.loads(json_content)
                logger.info("JSON fixed with targeted cleaning")
            except json.JSONDecodeError as e2:
                logger.error(f"JSON still invalid after targeted cleaning: {e2}")
