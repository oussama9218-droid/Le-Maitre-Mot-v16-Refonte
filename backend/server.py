...
{{
  "exercises": [
    ...
    try:
        ...
        # MINIMAL JSON cleaning - avoid breaking working content
        try:
            data = json.loads(json_content)
        except json.JSONDecodeError as e:
            logger.error(f"JSON parsing error: {e}")
            logger.error(f"Problematic JSON (first 300 chars): {json_content[:300]}...")
            
            # If JSON is completely broken, use fallback exercises
            logger.warning("Using fallback exercise generation due to JSON errors")
