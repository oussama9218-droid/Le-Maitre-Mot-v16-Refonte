#!/usr/bin/env python3
"""
Comprehensive Testing of 6e Generators (Waves 1, 2 & 3)
...
class SixeGeneratorsTester:
    ...
    def test_perimetres_aires_4_types(self):
        ...
        for i in range(20):
            ...
            if status_code == 200 and isinstance(response, dict):
                generator_code = response.get('generator_code', 'UNKNOWN')
                is_fallback = response.get('is_fallback', True)
        ...
        # Verify expected generator types
        expected_generators = ['PERIMETRE_AIRE', 'RECTANGLE', 'AIRE_TRIANGLE', 'AIRE_FIGURES_COMPOSEES']
        ...
        has_mix = len(expected_found) >= 2  # At least 2 different types
        ...
        success = successful_calls >= 15 and has_mix and all_non_fallback
    ...
    def test_angles_3_types(self):
        ...
        # Verify expected generator types
        expected_generators = ['ANGLE_MESURE', 'ANGLE_VOCABULAIRE', 'ANGLE_PROPRIETES']
        found_generators = list(generator_counts.keys())
        expected_found = [gen for gen in expected_generators if gen in found_generators]
        has_mix = len(expected_found) >= 2
        ...
        success = successful_calls >= 12 and has_mix
    ...
    def test_fractions_2_types(self):
        ...
        # Verify expected generator types
        expected_generators = ['CALCUL_FRACTIONS', 'FRACTION_REPRESENTATION']
        found_generators = list(generator_counts.keys())
        expected_found = [gen for gen in expected_generators if gen in found_generators]
    ...
    def test_proportionnalite_3_types(self):
        ...
        # Verify expected generator types
        expected_generators = ['PROPORTIONNALITE', 'PROP_TABLEAU', 'PROP_ACHAT']
        found_generators = list(generator_counts.keys())
        expected_found = [gen for gen in expected_generators if gen in found_generators]
        has_mix = len(expected_found) >= 2
    ...
    def test_nombres_entiers_3_types(self):
        ...
        # Verify expected generator types
        expected_generators = ['CALCUL_DECIMAUX', 'NOMBRES_LECTURE', 'NOMBRES_COMPARAISON']
        found_generators = list(generator_counts.keys())
        expected_found = [gen for gen in expected_generators if gen in found_generators]
        has_mix = len(expected_found) >= 2
    ...
    def test_geometrie_plan_5_types(self):
        ...
        expected_generators = ['TRIANGLE_CONSTRUCTION', 'QUADRILATERES', 'PROBLEME_2_ETAPES']
        found_generators = list(generator_counts.keys())
        expected_found = [gen for gen in expected_generators if gen in found_generators]
        has_mix = len(expected_found) >= 2
        ...
        success = successful_calls >= 15 and has_mix
    ...
    def test_enonce_quality_new_generators(self):
        ...
        for test in quality_tests:
            ...
            while not found_target and attempts < max_attempts:
                ...
                if status_code == 200 and isinstance(response, dict):
                    generator_code = response.get('generator_code', '')
    ...
    def _check_html_tables(self, generator_code, enonce_html, response):
        ...
        target_generators = ['PROP_TABLEAU', '6e_PROP_TABLEAU']
    ...
    def _check_writing_instructions(self, generator_code, enonce_html, response):
        ...
        target_generators = ['NOMBRES_LECTURE', '6e_NOMBRES_LECTURE']
        target_found = any(gen in generator_code for gen in target_generators)
    ...
    def _check_svg_visualization(self, generator_code, enonce_html, response):
        ...
        target_generators = ['FRACTION_REPRESENTATION', '6e_FRACTION_REPRESENTATION']
        target_found = any(gen in generator_code for gen in target_generators)
